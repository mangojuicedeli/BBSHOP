...
@Controller
@Log4j
@SessionAttributes({"id", "access_token"})
...
//		if (vo.getMEMBER_PW().equals(db_pw)) {
...
//	}
	...
	public String kakaoLogin(String code, Model model) {
		...
		// 전달 받은 인증 코드로 엑세스 토큰을 받아 온다.
		String access_token = kakaoAPI.getAccessToken(code);
		// 엑세스 토큰을 통해 카카오 사용자 조회 API를 호출해서 카카오에 저장된 사용자 정보를 얻어 온다.
		KakaoAccount user = kakaoAPI.getUserInfo(access_token);
		// 얻어온 사용자 정보 중 id와 엑세스 토큰을 model에 저장한다. (@SessionAttributes로 인해 session에 저장된다)
